...
class Ui():
    ...
    def main(self,bruh:ft.Page):
        ...
        self.bgfilepick = ft.ElevatedButton("切换背景图片", icon=ft.icons.UPLOAD_FILE,on_click=lambda _: self.pick_files_dialog.pick_files(allow_multiple=False,file_type="IMAGE"))
        #切换背景图片按钮
        ...
        #背景不透明度滑条
        self.yiyanbtn = ft.ElevatedButton("加载外部一言文件", icon=ft.icons.UPLOAD_SHARP,on_click=lambda _: self.yiyan_pick_files_dialog.pick_files(allow_multiple=False,allowed_extensions=["txt"]))
        #一言加载
        
        self.zitibtn = ft.ElevatedButton("更换显示字体", icon=ft.icons.UPLOAD_SHARP,on_click=lambda _: self.font_pick_files_dialog.pick_files(allow_multiple=False,allowed_extensions=["ttf"]))
        #自定义字体切换
        ...
        self.RunFullSC_btn = ft.FilledTonalButton("长按运行全屏广播命令",on_long_press=self.dic_RunFullSC,icon=ft.icons.FULLSCREEN)
        ...
        self.RunFullSC_swc = ft.Switch(label="Ctrl+Alt+F 以全屏运行广播命令",on_change=self.HotKey_RunFullSCR,active_color="pink")
        ...
        destinations=[
            ...
            ft.NavigationRailDestination(
            icon=ft.icons.FAVORITE_BORDER_OUTLINED, selected_icon_content=ft.Icon(ft.icons.FAVORITE,color="red"), label="关于"
        )
    ],
